...
import payIcon from "@/assets/payment/all_pay_icon.png"
import deliveryExpress from "@/assets/payment/delivery_express.png"
...
export default function CartCheckoutPage() {
    ...
    const isCodAllowed = cartItems.length > 0 && cartItems.every(item => item.payment_type == 0 || item.payment_type == undefined);
    //    const isPartialPayment = cartItems.some(item => item.payment_type == 1);
    const isFullPayment = cartItems.some(item => item.payment_type == 2);
    const isPartialPayment = !isFullPayment && cartItems.some(item => item.payment_type == 1);

    const [partialInputAmount, setPartialInputAmount] = useState(0)




    useEffect(() => {
        if (!isCodAllowed && !selectedPaymentMethod === "cod" || isFullPayment) {
            setSelectedPaymentMethod("sslcommerz")
        }
    }, [isCodAllowed, isFullPayment])
    ...
    const hasExpressSupport = cartItems.some(item => item.delivery_express_support == 1);
    const expressDeliveryChargeValue = cartItems.reduce((max, item) => Math.max(max, item.delivery_express || 0), 0);
    const partialPaymentAmount = cartItems.reduce((max, item) => Math.max(max, item?.partial_amount || 0), 0);

    useEffect(() => {
        if (partialPaymentAmount > 0) {
            setPartialInputAmount(partialPaymentAmount);
        }
    }, [partialPaymentAmount]);


    // const partialPaymentAmount = cartItems.reduce((sum, item) => sum + (Number(item.partial_amount) || 0) * item.quantity, 0);

    // const maxItem = cartItems.reduce((max, item) => {
    //   return (Number(item.partial_amount) || 0) >
    //     (Number(max.partial_amount) || 0)
    //     ? item
    //     : max;
    // }, { partial_amount: 0, quantity: 0 });

    // const partialPaymentAmount =
    //   (Number(maxItem.partial_amount) || 0) * maxItem.quantity;

    // console.log(partialPaymentAmount);


    const currentShippingCharge = isExpressDelivery ? expressDeliveryChargeValue : shippingCharge;

    // Calculate Grand Total
    let grandTotal = cartSubtotal + currentShippingCharge - discountAmount
    ...
    const validateCheckoutForm = () => {
        ...
        if (isPartialPayment) {
            if (!partialInputAmount || isNaN(partialInputAmount) || Number(partialInputAmount) < partialPaymentAmount) {
                return `Partial payment amount cannot be less than the minimum required ${partialPaymentAmount}৳`;
            }
            if (Number(partialInputAmount) > grandTotal) {
                return `Partial payment amount cannot exceed the grand total of ${grandTotal}৳`;
            }
        }
    ...
    const applyPromoCode = async () => {
        ...
        try {
            ...
            if (data?.discount_type == 0) {
                const calculatedDiscount = (cartSubtotal * data?.discount) / 100
                if (calculatedDiscount >= data?.max_discount) {
                    setDiscountAmount(data?.max_discount)
    ...
    const handlePlaceOrder = async () => {
        ...
        if (!errorMessage) {
            const orderData = {
                shipping_cost: currentShippingCharge,
                promo_code: cuponCode || null,
                description: "",
                payment_method: selectedPaymentMethod,
                payment_type: isPartialPayment ? "partial" : "others",
                partial_input_amount: isPartialPayment ? partialInputAmount : null,
    ...
    return (
          ...
          sm:rounded-lg sm:shadow-sm">
                        ...
                        <div className="border-t border-gray-200 pt-2 space-y-1">
                            ...
                            <div className="flex text-sm  justify-between text-gray-700">
                                ...
                                <span className="font-semibold">{currentShippingCharge.toFixed(2)}৳</span>
                            </div>
                ...
                <div className="flex text-sm  justify-between text-gray-700">
                  ...
                  rounded-r-md"
                        ...
                        {isPartialPayment && (
                            <div className="mt-3 p-3 bg-emerald-50/50 border border-dashed border-[#3A9E75] rounded-md">
                                <label htmlFor="partialInputAmount" className="block text-sm font-semibold text-gray-700 mb-1">
                                    Set Partial Payment Amount For order
                                </label>
                                <div className="flex gap-2 items-center">
                                    <div className="relative flex-grow rounded-md shadow-sm">
                                        <input
                                            type="number"
                                            name="partialInputAmount"
                                            id="partialInputAmount"
                                            className="focus:ring-[#3A9E75] focus:border-[#3A9E75] block w-full pl-3 pr-8 py-1.5 border border-gray-300 rounded-md bg-white text-sm"
                                            placeholder={`Min: ${partialPaymentAmount}৳`}
                                            value={partialInputAmount || ""}
                                            onChange={(e) => setPartialInputAmount(Number(e.target.value))}
                                        />
                                ...
                                <span className="text-[11px] text-gray-500 mt-1 block">
                                    Min: {partialPaymentAmount}৳ | Max: {grandTotal}৳
                                </span>
                            </div>
                        )}
                        ...
                        <div className="mt-1.5 ">
                            {hasExpressSupport && (
                                <div className="flex justify-between  border-b border-dashed border-gray-200 pb-2">
                                    <div className="items-center gap-2 py-3">
                                        <div className="flex gap-2">
                                            <input
                                                ...
                                                className="h-4 w-4 cursor-pointer accent-[#3A9E75]"
                                            />
                                            <label htmlFor="expressDelivery" className="text-sm font-medium text-gray-700 cursor-pointer">
                                                Express Delivery  ( {expressDeliveryChargeValue}৳ )
                                            </label>
                                        </div>

                                    </div>
                                    <div className="items-center ">
                                        <img src={deliveryExpress.src} alt="Express Delivery" className="h-12" />
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between">
                                    ...
                                    <div className=" flex flex-wrap gap-3  justify-between items-center ">
                                        ...
                                        <img src={payIcon.src} alt="Payment Methods" className="h-12" />
                                    </div>
                                </div>

                                {(!isFullPayment && !isPartialPayment) && (
                                    <div className="flex items-center ">
                                        <input type="radio" id="cod" name="paymentMethod" value="cod" checked={selectedPaymentMethod === "cod"} onChange={(e) => setSelectedPaymentMethod(e.target.value)} className="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" />
                                        <label htmlFor="cod" className="ml-3 block text-base font-medium text-gray-700">
                                            Cash on Delivery
                                        </label>
                                    </div>
      ...
      focus:outline-none focus:ring-2`}
                            >
                                {loading ? "Loading.." : (selectedPaymentMethod === 'cod' ? "Place Order" : ((isPartialPayment && !isFullPayment) ? `Pay Partial payment ( ${partialInputAmount} )` : "Pay Now"))}
                            </button>
                        </div>
